...
> ⚠️ **同名顶层 key 跨文件也会冲突**。资源管理器会严格拒绝重复节点；使用 schema 校验并通过完整 Client/CLI 加载验证。
...
项目使用 `maa-framework-go/v4`。在 Custom runner 中调用 `ctx.GetNode()` 获取 override 后的强类型节点，或调用 `ctx.GetNodeJSON()` 获取原始 JSON。
...
```go
// 当前 Custom Action 直接读取注入后的参数。
var param struct {
    Stage string `json:"stage"`
}
err := json.Unmarshal([]byte(arg.CustomActionParam), &param)

// 读取其他 Custom Action 节点时断言强类型参数。
actionParam, ok := node.Action.Param.(*maa.CustomActionParam)
if !ok {
    return false
}
```
