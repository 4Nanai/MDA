...
- `expected` 数组按**或**逻辑匹配，任一候选命中即可；不要把数组成员误当作必须同时命中。
- OCR 易误识别个别字时，用正则跳过不稳定字、保留可靠首尾锚点。例如 `"时.*取"` 可覆盖 `时装抽取` 与 `时装描取`。
- 不要保留被更宽正则完全覆盖的候选；在小 ROI 且同时要求模板等第二条件时，可安全使用更宽的正则。为每个正则加 `// @i18n-skip`。
